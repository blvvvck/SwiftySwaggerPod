...
public struct SwaggerNumericSchema<T: Codable & Comparable>: Codable, Equatable {
    ...
    public var minimum: SwaggerSchemaValueBound<T>?

    /// Upper bound of possible values.
    public var maximum: SwaggerSchemaValueBound<T>?
    ...
    public init(from decoder: Decoder) throws {
        ...
        minimum = try container.decodeIfPresent(T.self, forKey: .minimumValue).map { minimumValue in
            return SwaggerSchemaValueBound(
                minimumValue,
                isExclusive: try container.decodeIfPresent(Bool.self, forKey: .isMinimumExclusive)
            )
        }

        maximum = try container.decodeIfPresent(T.self, forKey: .maximumValue).map { maximumValue in
            return SwaggerSchemaValueBound(
                maximumValue,
                isExclusive: try container.decodeIfPresent(Bool.self, forKey: .isMaximumExclusive)
            )
        }
    }
